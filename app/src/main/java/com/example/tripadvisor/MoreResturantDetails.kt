...
class MoreResturantDetails : AppCompatActivity() {
    ...
    private fun init() {
        ...
        if (Id != -1) {
            GlobalScope.launch {
                try {
                    Log.d("MoreResturantsDetails", "Starting API call ID:${Id}")

                    val data= db.getResturantDetailByResturantId(Id)
                    val datahotel= db.getResturantByID(Id)

                    Log.d("MoreRestutrantsDetails", "API call successful: ${data} MoreResturantDetails fetched")
    ...
    private fun bindHotelData(datahotel:resturant,hotel: resturantDetails) {
        // Assuming you have a method to load images from a URL
        // You might use libraries like Glide or Picasso
        val imgHotel: ImageView = findViewById(R.id.imgHotel)

        val path=datahotel.images.replace("\\", "/")
...
//
//        val imageNames = datahotel.images.split(",").map { it.trim() }
//        if (imageNames.isNotEmpty()) {
//            // Select a random image from the list
//            val imageResId = this.resources.getIdentifier(
//                imageNames.random(), "drawable", this.packageName
//            )
//            imgHotel.setImageResource(imageResId)
//            // Use randomImageName as needed
//        } else {
//            // Handle the case where no images are available
//            Log.d("ExploreFragment", "No images available")
//        }
        ...
        findViewById<TextView>(R.id.websiteLink).text = "Visit the website"
...
//         Set the email link
        findViewById<TextView>(R.id.emailLink).text = "Menu"
        findViewById<TextView>(R.id.emailLink).setOnClickListener {
            // Handle the click to send an email
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse(datahotel.menu))
            startActivity(intent)
        }
...
//            val intent = Intent(Intent.ACTION_VIEW)
//            intent.data = Uri.parse(datahotel.)
//            this.startActivity(intent)
//        }
...
//            val emailAddress = hotel. // Assume `hotel` has the email address
